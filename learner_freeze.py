...
class LearnerFreeze(nn.Module):
    def __init__(self, config):
        """
        param config: network config file, type:list of (string, list)
        """
        super(LearnerFreeze, self).__init__()
        self.config = config
    ...
    def forward(self, x, net_vars=None, bn_training=True):
        ...
        for name, param in self.config:
            if name is 'conv2d':  # remember to keep synchronized of forward_encoder and forward_decoder!
                ...
                w_re.requires_grad, b_re.requires_grad, w_img.requires_grad, b_img.requires_grad = False, False, False, False
                ...
                w.requires_grad, b.requires_grad, running_mean.requires_grad, running_var.requires_grad = False, False, False, False
